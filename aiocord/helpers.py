...
def mime_from_bytes(image_bytes: bytes):
    if image_bytes.startswith(b'\xFF\xD8') and bytes.rstrip(b'\0').endswith(b'\xFF\xD9'):
        return 'image/jpeg'

    elif image_bytes.startswith(b'\x89\x50\x4E\x47\x0D\x0A\x1A\x0A'):
        return 'image/png'

    elif image_bytes.startswith((b'\x47\x49\x46\x38\x37\x61', b'\x47\x49\x46\x38\x39\x61')):
        return 'image/gif'
