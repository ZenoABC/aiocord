...
class Cache:
    ...
    def ready(self, user_data, channels_data, guilds_data):
        ...
        storage.update_dict(storage.Channel, ("id",),
                            self._channels, channels_data)
        guild_ids = (guild_data["id"] for guild_data in guilds_data)
        storage.update_dict(storage.Guild, ("id",), self._guilds, guilds_data)
        return guild_ids
    ...
    def guild_emojis_update(self, guild_id, emojis_data):
        ...
        storage.update_dict(storage.Emoji, ("id",), guild.emojis, emojis_data)
    ...
    def message_reaction_add(
        self, guild_id, channel_id, message_id, user_id, emoji_data
    ):
        if guild_id:
            guild = self._guilds[guild_id]
            channels_store = guild.channels
    ...
    def voice_state_update(self, guild_id, channel_id, user_id, state_data):
        guild = self._guilds[guild_id]
        if channel_id:
